...
export const DataProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        // 2. Initialize Cornerstone
        // 3a. Initialize Supabase Client
        // 3b. Initialize Supabase Auth and get User Data (anonymous or logged in)
        // 4. If a sharing key is on URL at startup, place that into state after the above 
        ...
        case 'sharer_status_changed':
            ...
            if (usersWhoAreSharing.length > 0) {
                ...
                if (nonRecentSharers.length !== 0) {
                    ...
                    toast(`${mostRecentShare.discordData.username} has requested control`);
                    new_data = { ...data, sharingUser: null, activeUsers: globalSharingStatus };
                } else {
                    //toast(`"${mostRecentShare.user} has taken control`);
                    toast(`${mostRecentShare.discordData.username} has taken control`);
